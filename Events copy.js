...
const EVENT_RESPONSE_MAPPING = Object.freeze([
    ...
    { index: 16, key: 'npc_arrival_departure', postProcess: entry => {
        if (entry && typeof entry === 'object' && !entry.action) {
            entry.action = 'left';
        }
        return entry;
    } },
    { index: 17, key: 'npc_arrival_departure', postProcess: entry => {
        if (entry && typeof entry === 'object' && !entry.action) {
            entry.action = 'arrived';
        }
        return entry;
    } },
...
class Events {
    ...
    static initialize(deps = {}) {
        ...
        this._entryParsers = {
            ...
            harvestable_resource_appear: raw => this.splitVerticalBarEntries(raw),
            npc_first_appearance: raw => this.splitVerticalBarEntries(raw),
        ...
        this._handlers = {
            ...
            harvestable_resource_appear: (entries, context) => this.handleItemAppearEvents(entries, context),
    ...
    static parseEventCheckResponse(responseText) {
        ...
        for (const mapping of EVENT_RESPONSE_MAPPING) {
            const raw = numberedEntries.has(mapping.index) ? numberedEntries.get(mapping.index) : '';
            if (!grouped.has(mapping.key)) {
                grouped.set(mapping.key, []);
            }
            grouped.get(mapping.key).push({ raw, mapping });
        }

        const rawEntries = {};
        const parsedEntries = {};

        const applyPostProcess = (value, mapping) => {
            if (!mapping || typeof mapping.postProcess !== 'function') {
                return value;
            }
            const candidate = value && typeof value === 'object' ? { ...value } : value;
            return mapping.postProcess(candidate);
        };

        const combineParsedChunks = (key, chunks, mappings) => {
            if (key === 'currency') {
                const numericValues = chunks
                    .filter(value => Number.isFinite(value))
                    .map(value => Number(value));
                if (!numericValues.length) {
                    return null;
                }
                return numericValues.reduce((total, amount) => total + amount, 0);
            }
            ...
            chunks.forEach((chunk, idx) => {
                if (Array.isArray(chunk)) {
                    chunk.forEach(item => {
                        ...
                        aggregated.push(applyPostProcess(item, mappings[idx]?.mapping));
                    });
                } else if (chunk !== null && chunk !== undefined) {
                    aggregated.push(applyPostProcess(chunk, mappings[idx]?.mapping));
                }
            });
        ...
        for (const [key, segments] of grouped.entries()) {
            ...
            rawEntries[key] = combinedRaw || '';

            const parser = this._entryParsers[key];
            if (!parser) {
                parsedEntries[key] = rawEntries[key];
                continue;
            }

            const chunks = segments.map(({ raw }) => parser(raw));
            const combinedParsed = combineParsedChunks(key, chunks, segments);

            if (Array.isArray(combinedParsed)) {
                parsedEntries[key] = combinedParsed.filter(entry => entry !== null && entry !== undefined);
            } else {
                parsedEntries[key] = combinedParsed;
            }
        }

        // Ensure all expected keys exist even if empty so downstream logic can rely on presence.
        for (const { key } of EVENT_RESPONSE_MAPPING) {
            if (!Object.prototype.hasOwnProperty.call(rawEntries, key)) {
                rawEntries[key] = '';
            }
            if (!Object.prototype.hasOwnProperty.call(parsedEntries, key)) {
                parsedEntries[key] = key === 'currency' ? null : [];
            }
        }

        const firstAppearanceNames = Array.isArray(parsedEntries.npc_first_appearance)
            ? parsedEntries.npc_first_appearance
            : [];

        if (firstAppearanceNames.length) {
            const arrivals = firstAppearanceNames
    ...
    static async applyEventOutcomes(parsedEvents, context = {}) {
        ...
        const suppressedItemEvents = omitItemGeneration ? new Set(['scenery_appear', 'harvestable_resource_appear', 'item_appear', 'alter_item']) : null;
    ...
    static async runEventChecks({ textToCheck, stream = null, allowEnvironmentalEffects = true, isNpcTurn = false } = {}) {
        ...
        try {
            ...
            if (typeof promptEnv?.render !== 'function' || typeof parseXMLTemplate !== 'function') {
                throw new Error('Event check dependencies missing prompt rendering utilities.');
            }

            if (typeof this.deps?.prepareBasePromptContext !== 'function') {
                throw new Error('Event check handler missing base context builder.');
            }
            ...
            if (!parsedTemplate?.systemPrompt || !parsedTemplate?.generationPrompt) {
                throw new Error('Event check template missing prompts.');
            }
            ...
            const cleanedResponse = this.cleanEventResponseText(responseText);
            const htmlResponse = this.escapeHtml(cleanedResponse).replace(/\n/g, '<br>');

            const structured = this.parseEventCheckResponse(responseText);
            if (!allowEnvironmentalEffects) {
                ...
                if (
                    ...
                    Object.prototype.hasOwnProperty.call(structured.rawEntries, 'environmental_status_damage')
                ) {
