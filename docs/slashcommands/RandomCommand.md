...
- The narrative pipeline renders `base-context.xml.njk` with `promptType: 'random-event'`. With the `random_event` TinyBrain family enabled it plans travel scope, drafts, audits, optionally revises, and strictly parses a `<turnResult>` or full `<moveTurnResult>` final in one staged conversation; otherwise it uses the existing one-shot completion. It then stores a `type: 'random-event'` assistant entry, runs event checks with time advancement suppressed, summarizes the entry, and records event/status summary entries.
