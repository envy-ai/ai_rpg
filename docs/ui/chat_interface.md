...
- `initQuestPanel()` uses `/api/quest/edit` and `/api/player/quests/:id`.
  ...
  - Quest edit modal supports NPC disposition rewards via multiline `NPC name: disposition +/-intensity - reason` input; unknown NPCs are skipped server-side with a console warning.
