...
- **Location panel** (`.location-block`):
  ...
  - Image + upper-left favorite toggle plus context menu for edit/summon/regenerate. The favorite toggle is a borderless transparent star button inset `4px` from the image corner, uses the outline/solid star assets, writes `Location.favorite` through `PUT /api/locations/:id/favorite`, and the saved marker drives the Favorites subtab.
...
- `#craftingModal` (crafting/processing via `/api/craft`, plus `modify-location` mode via `/api/locations/:id/modify`, including empty-slot submits for craft/process/modify-location, no-prose submit paths, notes placeholders that mention `<N>` roll override support, and Ctrl/Cmd+Enter primary prose submits from the notes field).
  ...
  - Equipped items in the crafting inventory are highlighted with a red outline and are rejected from slot assignment across drag/drop, double-click, and keyboard assignment with a `must be unequipped first` alert; non-empty containers are greyed out, cannot be dragged into crafting slots, and are rejected from slot assignment until emptied. The non-empty check uses normalized, nonblank contained thing ids and consults the latest cached container payload, so a container emptied through the container modal can be selected even if the crafting picker was opened before the transfer finished.
