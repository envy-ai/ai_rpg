...
- Streamed NPC turns can include the same tool-derived attack summaries, action resolutions, and plausibility payload arrays as player actions; the client renders those as attack-check, skill-check, and plausibility insight messages/buttons while the persisted attachment entries keep the final chat-history view linked to the NPC prose.
...
- `resolveAttack` tool calls are recorded as `attack-check` attachments on the assistant response, so each attack call in a turn gets its own top-right hover insight button.
- `resolvePlausibilityCheck` and `resolveOpposedPlausibilityCheck` tool calls can return `actionResolutions[]` and `plausibilities[]`; the live chat handler renders each as the existing skill-check and plausibility hover insight buttons, and persisted history stores them as linked attachment entries.
