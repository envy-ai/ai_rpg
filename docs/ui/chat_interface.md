...
- **Location panel** (`.location-block`):
  ...
  - Items/Scenery grids + "Craft" and "New Item/Scenery" buttons. In the New Item/Scenery create modal, the Slot pulldown includes a blank `Not specified` option that omits slot guidance from the generation seed.
...
- `#craftingModal` (crafting/processing via `/api/craft`, plus `modify-location` mode via `/api/locations/:id/modify`, including empty-slot submits for craft/process/modify-location, no-prose submit paths, and notes placeholders that mention `<N>` roll override support).
  ...
  - Equipped items in the crafting inventory are highlighted with a red outline and are rejected from slot assignment across drag/drop, double-click, and keyboard assignment with a `must be unequipped first` alert; non-empty containers are greyed out, cannot be dragged into crafting slots, and are rejected from slot assignment until emptied.
