...
- **Chat panel** (`.chat-container`):
  ...
  - `requestUserInput` chat-tool calls show `#playerInputRequestPanel`, a draggable non-modal floating question panel shared across all tabs with the same client id. It posts answers/cancellations to `/api/chat/user-input-response` and has no backdrop, so Adventure, map, and Story Tools content remain clickable while the player checks context.
