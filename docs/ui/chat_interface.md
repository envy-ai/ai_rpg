...
- Tab bar controls panels with ids `tab-adventure`, `tab-map`, `tab-world-map`, `tab-favorites`, `tab-character`, `tab-quests`, `tab-factions`, `tab-party`, `tab-story-tools`.
...
- **Location panel** (`.location-block`):
  ...
  - Image + upper-left favorite toggle plus context menu for edit/summon/upload/regenerate. The favorite toggle is a borderless transparent star button inset `4px` from the image corner, uses the outline/solid star assets, writes `Location.favorite` through `PUT /api/locations/:id/favorite`, and the saved marker drives the Favorites subtab.
  ...
  - Item tooltips render stored mechanical fields such as attribute bonuses and equipper status effects even when an item is not normal slot-based gear, so mod-installed items can still expose their stats.
  ...
  - Weapon classification and tooltip type labels use top-level `itemTypeDetail` first, then metadata mirrors including legacy generated `metadata.itemType`, so older event-created weapons still show weapon-specific tooltip data.
  ...
  - The shared item/scenery edit modal renders registered Thing fields with `exposeToEditModal`, including the implants mod's `Implant slot` field and the modules mod's slot/module controls. Values are saved as first-class Thing fields, not metadata.
    - Things marked `Container` show an `Open Container` context-menu action and container badge action. Containers with `requiresCheckToOpen` add a small red lock overlay to the upper-right corner of the container badge on item/scenery cards. The container modal uses two shared thing-list panels: current player inventory on the left and the selected container contents on the right, with persisted view keys `containerPlayerInventory` and `containerContents`; on narrow/mobile layouts those panels split the modal vertically with player inventory in the top half and container contents in the bottom half. If the opened container still has pending generated `containerContents` seeds, the contents panel shows a spinner with `Container contents are being generated...` until the container inventory response returns, instead of briefly showing the empty-container text.
    ...
    - Dropping a loose module item card onto a modular item card installs it when the target has a matching open slot. This uses the existing modules install action, supports inventory and current-location loose module sources, and consumes the drop before the generic location-drop handler can treat the module as an ordinary item drop.
...
- **Chat panel** (`.chat-container`):
  ...
  - `requestUserInput` chat-tool calls show `#playerInputRequestPanel`, a draggable non-modal floating question panel shared across all tabs with the same client id. It posts answers/cancellations to `/api/chat/user-input-response` and has no backdrop, so Adventure, map, and Story Tools content remain clickable while the player checks context. Destructive chat tools such as `deleteThing` reuse the same panel in confirmation mode: the text area is hidden, the title and confirm/cancel labels come from the server payload, and the panel still stays modeless.
...
- **Sidebar** (`.chat-sidebar`):
  ...
  - Player "View" fetches fresh `/api/player` data before opening `#npcViewModal`, so tool-driven state changes such as mod status sections are visible even if the sidebar cache is stale. If the modal then fetches detailed character status for attributes, it preserves incoming mod status sections when the detail response is partial. The modal opens in editable mode for attributes/skills using shared allocation partials; skills can now be added/removed directly in this modal for the player view. Above the base attribute controls, a Calculated Attributes grid shows detailed `attributeInfo.modifiedValue` totals in green, red, or white depending on whether each value is above, below, or equal to base. The modal now includes a Faction section above Equipment: NPCs show a single faction, while player view lists all factions with reputation tier labels and associated tier perks. NPCs use the same allocation sections in read-only mode, show server-derived unspent point totals, and abilities render as read-only cards matching the level-up ability selector style with active/passive/triggered type color coding.
  - Player "Abilities" fetches fresh `/api/player` data before opening `#playerAbilitiesModal`, a no-backdrop dialog with `aria-modal="false"` that renders the player's abilities with the same `.npc-view-ability-card` format used by the View modal. Clicking an ability inserts `[Ability Name]` at the current cursor/selection in `#messageInput` and refocuses the chat entry window. Pressing `A` opens the modal when focus is not inside an input, textarea, select, contenteditable area, or chat edit textarea.
...
- Player/NPC "Edit" opens `#npcEditModal`, which includes an `Aliases` textarea (one alias per line) plus `Resistances`/`Vulnerabilities` textareas and saves through `PUT /api/npcs/:id`.
  ...
  - Player/NPC Inventory modal keeps active inventory filters (including slot filter selection) when equip/unequip triggers an inventory re-render, hides installed modules as standalone item entries while still caching them for base-item tooltip/lightbox details, and serves as the shared thing-list card/filter implementation that location item/scenery sections and the crafting inventory now reuse. The player inventory button also fetches fresh `/api/player` data before opening so newly installed mod-owned systems and inventory-side updates are reflected.
  - Party summary list.
- **Favorites tab** (`#tab-favorites`):
  ...
  - Each card shows the generated location image when available, location name, region, computed shortest-route travel time from the current location, and short description. The current location is shown as `Current location` and disabled rather than dispatching a no-op travel action; favorites with no directed route show `No known route`.
  - Clicking another favorite activates the Adventure tab and calls the same `window.travelToAdjacentLocationFromMap(locationId, { focusAdventureTab: false })` path used by the world map.
...
- `prompt_progress`, `prompt_progress_cleared` (docked prompt tracker rendered as `#promptProgressDock` between `#chatLog` and `.input-area`; display state persists under `airpg:promptProgressDockState` and defaults to `one-line`. `collapsed` is a 4px glowing yellow aggregate progress bar that expands to `one-line` on click, `one-line` uses the progress fill as the row background while showing the longest-running prompt with tightly spaced transparent white SVG view/restart/cancel controls to the left of its name, received characters, and floored approximate percent such as `~42%`, and `table` shows the full table with a sticky header, no `Target`, `Avg Out`, `Runs`, or `Avg/s` columns, and a vertical scrollbar capped to three body rows. Prompt action icons stay fully bright on hover/active and gain a slight white drop-shadow glow. At non-mobile widths, the one-line prompt name uses a fixed 60% row allocation with ellipsis overflow, renders at font weight 600, and appends `(and N more)` when other prompts are running. The one-line and table mode controls use right-aligned white `assets/material-icons/misc/compress.svg` and `assets/material-icons/misc/expand.svg`; one-line compress returns to the bar, one-line expand opens the table, and table compress returns to one-line. The dock remains visible with an italic translucent `no prompts running` idle row, and the one-line idle state hides the prompt-specific actions and status text. The view control spawns a new modeless floating prompt/response viewer with `Copy Prompt` and `Follow`; viewers have no backdrop, keep the rest of the page interactive, can coexist, keep their last prompt/response snapshot after live progress clears, and close only from their own close buttons. When a viewer's `Follow` checkbox is checked, that combined pane stays scrolled to the bottom on throttled viewer refreshes. Server received counts are decoded characters for both OpenAI-compatible and Codex bridge streams, and payloads include `progressFraction`, `targetCharacters`, `runCount`, and `averageOutputCharacters`; positive established averages become the next run's `targetCharacters`, otherwise config targets are used. Completed prompts briefly emit `progressFraction: 1` for a 250 ms hold with a brighter fade-out pulse. Progress updates and rerenders are throttled to 500 ms).
...
- `#craftingModal` (crafting/processing via `/api/craft`, plus `modify-location` mode via `/api/locations/:id/modify`, including empty-slot submits for craft/process/modify-location, no-prose submit paths, notes placeholders that mention `<N>` roll override support, and Ctrl/Cmd+Enter primary prose submits from the notes field).
  - The left-side crafting picker lists active player inventory items, current-location items and scenery, and item contents inside current-location containers, using the same shared thing-list renderer as the main inventory and location panels, including per-panel `Classic`/`Table`/`Grid` views, search/slot/equipment filters, and the shared stable sort popup, with narrow panels collapsing the filter controls behind the shared icon-only filter toggle. Contents from scenery containers are included, while nested container contents remain item-only.
  - The available-inputs section header now renders the `Available Items & Scenery` title/count row with the crafting hint directly beneath it, while the icon-only filter toggle remains a separate control on the right.
  - Equipped items in the crafting inventory are highlighted with a red outline. Double-click, click, and keyboard assignment still reject them with a `must be unequipped first` alert, while drag/drop asks to unequip the item before assigning it to a crafting slot and, on confirmation, unequips through the normal equipment endpoint before completing the assignment; non-empty containers are greyed out, cannot be dragged into crafting slots, and are rejected from slot assignment until emptied. The non-empty check uses normalized, nonblank contained thing ids and consults the latest cached container payload, so a container emptied through the container modal can be selected even if the crafting picker was opened before the transfer finished.
...
- `initQuestPanel()` uses `/api/quest/edit` and `/api/player/quests/:id`.
  - Quest edit modal supports quest giver display-name editing with NPC datalist suggestions, paused and reward-claimed checkboxes, per-item reward rows, per-faction reputation reward rows with faction selectors and signed integer deltas, and per-NPC disposition reward rows with NPC selectors, configured disposition-type dropdowns populated from the current player payload's `dispositionDefinitions`, signed integer intensity, and optional reason. NPC disposition reward rows place the reason field on a full-width second row and preserve existing custom type values as custom options.
