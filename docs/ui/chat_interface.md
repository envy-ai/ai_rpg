...
- `resolveSkillCheck` and `resolveOpposedSkillCheck` tool calls can return `actionResolutions[]` and `plausibilities[]`; the live chat handler renders each as the existing skill-check and plausibility hover insight buttons, and persisted history stores them as linked attachment entries.
