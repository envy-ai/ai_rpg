...
- `#craftingModal` (crafting/processing via `/api/craft`, plus `modify-location` mode via `/api/locations/:id/modify`, including empty-slot submits for craft/process/modify-location, no-prose submit paths, and notes placeholders that mention `<N>` roll override support).
  - The left-side crafting picker lists active player inventory items plus current-location items, using the same shared thing-list renderer as the main inventory and location panels, including per-panel `Classic`/`Table`/`Grid` views, search/slot/equipment filters, and the shared stable sort popup, with narrow panels collapsing the filter controls behind the shared icon-only filter toggle.
...
- `initQuestPanel()` uses `/api/quest/edit` and `/api/player/quests/:id`.
  - Quest edit modal supports per-item reward rows, per-faction reputation reward rows with faction selectors and signed integer deltas, and per-NPC disposition reward rows with NPC selectors, configured disposition-type dropdowns, signed integer intensity, and optional reason. NPC disposition reward rows place the reason field on a full-width second row and preserve existing custom type values as custom options.
  - NPC disposition reward rows submit structured reward entries; unknown NPCs are skipped server-side with a console warning.
  - Quest reward lists resolve faction reputation reward IDs to faction names and render NPC disposition rewards when present.
