...
- Notes: location and region edit forms share `views/_includes/vehicle-info-fields.njk` for vehicle editing (`isVehicle` toggle, `icon` dropdown, a single-select `currentDestination` typeahead, `ETA`/`vehicleExitId`, and a destinations add/remove picker with live substring suggestions over named locations).
