...
- Notes: location and region edit forms share `views/_includes/vehicle-info-fields.njk` for vehicle editing (`isVehicle` toggle, `icon` dropdown, a single-select `currentDestination` typeahead, `ETA`, and a `Vehicle Exit` select). Vehicle-exit options render as `inside -> outside`; location/stub editors list exits from the edited location, while region editors list cross-region exits from locations in that region. Map-tab stub editing reuses the same vehicle editor in stub mode.
