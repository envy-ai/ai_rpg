...
- Notes: if `/api/load` returns `MOD_ENABLEMENT_MISMATCH`, the client opens the Save Mods Differ modal. It lists newly added mods (`extraActive`: enabled now but absent from the save) and missing mods (`missingFromActive`: required by the save but inactive now) in separate groups. `Use Save Mods` calls `/api/mods/apply-save-config` and requires restart, `Keep Current Mods` retries `/api/load` with `modMismatchChoice: 'keep-current'`, and `Cancel Load` stops without mutating config.
...
- Notes: the Image Generation section includes prompt-batching controls for enabling the LLM prompt-writing debounce, setting the debounce delay, and setting the maximum compatible prompts per batch.
...
- Notes: mod setting fields with a registered tab render in their own World Profiles tab; legacy ungrouped mod fields still render in the Prompt Guidance `Mod Settings` block. Registered select fields render as dropdowns, array fields serialize through the generic mod-settings payload, and non-persisted `applyPreset` action fields ask for confirmation before copying preset values into editable persisted mod settings. The bundled modules mod uses a custom row editor for its `slotTypes` array.
