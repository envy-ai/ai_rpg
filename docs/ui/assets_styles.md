...
- `public/css/main.scss`
  ...
  - The relationship graph tab reuses the map tab sizing rules and adds `.relationship-graph-panel`, `.relationship-graph-status`, and `.relationship-graph-status--error` for its Cytoscape container and explicit loading/empty/error states.
  ...
  - The chat sidebar tracker list uses `.chat-trackers-*` and `.chat-tracker-card*` hooks. `percentage` and `x_out_of_total` tracker cards add `.chat-tracker-card--progress` plus `--tracker-progress-percent` so `main.scss` can draw a clamped display-only background progress fill under the name/value row. Tracker card edit/delete controls are absolutely positioned overlays with no reserved card padding, use the top local tracker-card stacking layer, then fade in on hover/focus.
...
- `cytoscape.min.js` plus `layout-base.js`, `cose-base.js`, `cytoscape-fcose.js`, `cytoscape-euler.js`, and `public/js/cytoscape-convex-hull.js` for Region Map, World Map, and Relationship Graph rendering.
...
- Portrait condition indicators share the level-stack slot just below the bare `L.<level>` text.
  ...
  `assets/material-icons/misc/health_blood.svg` plus the summed `-x` per-round loss. These
  indicators use the same black-outline treatment instead of a pill background, with icons
  scaled to the line height via `1em` sizing so they track their adjacent text.
