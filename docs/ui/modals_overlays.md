...
- `#mapFastTravelConfirmModal`: active modal for prompt-backed fast-travel confirmation. Region Map, World Map, and Favorites open it after previewing route time, then continue through `/api/chat` before teleporting with travel-time accounting. Character-menu story-tool teleports bypass it.
