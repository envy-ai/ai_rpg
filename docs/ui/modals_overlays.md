...
- `#calendarEditModal`: JSON editor for the active `calendarDefinition`, opened from the location/map context menus. It loads `/api/calendar`, saves through `PUT /api/calendar`, formats JSON client-side, and displays validation failures from the server without mutating the calendar.
