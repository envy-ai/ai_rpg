...
- Prompt-progress dock (`#promptProgressDock`, rendered in `views/index.njk` between `#chatLog` and `.input-area`, controlled by `public/js/chat.js`):
  ...
  - Each expanded table row includes progress fraction, run count, received characters, elapsed/timeout/latency/rate, retries, and eye/cancel/retry actions. Target-character and average-output values remain in the progress payload/stats, but the table does not display `Target` or `Avg Out` columns.
