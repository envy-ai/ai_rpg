...
- `scope.registerEntityValidator({ entityType, name, validator })` registers a whole-entity invariant. `validator(entity, context)` runs after field normalization and before every `ThingMutationService` commit; throwing rejects the complete candidate. Validators also run during the read-only hydration audit. Names are unique per mod/entity and failures remain explicit.
...
- Array/object fields can declare `xmlPrompt.collection` with `itemTagName` and child `fields` (`fieldName`, optional `tagName`, `type`, and `required`). The shared prompt renderer emits nested elements and the parser accepts only that declared structure. Nonempty text without the expected item elements and unexpected child tags fail structural validation.
...
- Dynamic `descriptionProvider` and `xmlPromptPlaceholderProvider` callbacks are evaluated when field snapshots are requested. Non-string provider return values throw; `null` and `undefined` keep the registered fallback text. When both properties reference the same callback, it is evaluated once per field snapshot and its result is shared across both prompt surfaces.
