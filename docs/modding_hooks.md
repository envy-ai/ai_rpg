...
- `toolSchema` optionally supplies the JSON schema used for that field in `createThing`. Use this for structured array/object fields that need item shapes, required keys, or `additionalProperties: false`; the registered description or `descriptionProvider` still supplies the active prompt text.
...
- Fields may provide dynamic `descriptionProvider` and `xmlPromptPlaceholderProvider` callbacks. The returned text is resolved when the current field snapshot is requested, so prompts and live tool schemas can include active world-profile options such as configured module slot types.
