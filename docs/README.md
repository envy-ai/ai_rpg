...
- [config.md](config.md) — Configuration options for gameplay behavior (including point pool formulas, AI `custom_args` payload injection with per-key override merging, extensible `random_event_frequency` file-based event types, world-time config, slop-remover base attempts, supplemental/offscreen NPC prompt cadence with turn-cap safety limits, and startup `--config-override` YAML layering).
...
- [server_llm_notes.md](server_llm_notes.md) — Deep notes on `server.js`, `api.js`, `Events.js`, and `LLMClient.js` responsibilities and flow (including ComfyUI init behavior, supplemental/offscreen/plot-summary hidden-prompt scheduling behavior plus missing-counter immediate-run fallback on load, calendar-generation prompt behavior including Earth-like => Gregorian guidance plus Gregorian fallback, season `timeDescriptions`, region weather parsing/persistence, compact end-of-base-context conditions with 24-hour time and weather-aware omission, faction relation neutral defaults plus missing-asset tolerance during generation, defensive region-entry stub controlling-faction reconciliation, load-time stale faction-reference reconciliation, retry-time AI override/config refresh in LLM calls, formula-driven NPC skill/attribute progression with budget logging, NPC race/class representation guidance plus contemplation steps in generation prompts, event-summary filtering, scene-summary fallback filtering for hidden/event-status blocks, scene-summarizer input filtering that keeps hidden entries while excluding event/status blocks, overlap-aware and compact-rendered scene-detail inclusion in older-story history, random-event prompt logging via `LLMClient.logPrompt`, and craft-history filtering).
...
- [ui/pages.md](ui/pages.md) — Route-to-template map with scripts, injected data, and key form notes (including default skills prefill, new-game skill add/remove plus start-time hour input, remote new-game form settings save/load, blank-safe attribute alias matching during load, master-detail settings manager layout with tabbed editor sections, stable tab pill sizing, container-scoped library/editor scrolling, search/sort library controls, region edit modal parent/faction field handling, settings rename/new-id behavior, persistent settings delete behavior, shared allocation partials, and allocation pool behavior/timing).
- [ui/chat_interface.md](ui/chat_interface.md) — Main chat UI layout, behavior, data flow, player-point warning indicator behavior, left-sidebar world-time indicator + transition summaries + unlabeled light-level line + conditional weather line + weather/light-threshold event summaries (with no-weather light-update suppression), player-view modal point allocation flow (including in-modal skill add/remove for player view and hidden unspent-point totals in non-player NPC views), inventory modal filter persistence across equip/unequip refreshes, floating prompt-progress overlay behavior (including contract/expand, header drag + resize handles, and a 3.5-second empty-state debounce via hidden placeholder row), location thing drag/drop behavior (item inventory drops plus item/scenery conversion between grids), faction creation modal + AI fill-missing precreate flow, LLM modal submit behavior (including no-prose craft/process/salvage/harvest submits), and location exit caching.
...
- [api/chat.md](api/chat.md) — Chat endpoints, sorted by path; documents travel-prose split event payloads (including move-turn `item_appear`/`scenery_appear` handling), world-time payloads, destination stub creation, `?` question-action routing (`user-question`/`storyteller-answer` entries, slop-remover bypass), `@`/`@@`/`@@@` generic prompt routing with persistence modes, server-only supplemental/offscreen/plot-summary hidden story-note entries and cadence rules (including server-side NPC relocation when offscreen activity marks `moved=true`), and references shared payloads in `docs/api/common.md`.
...
- [api/crafting.md](api/crafting.md) — Crafting endpoints (including `timeTaken`-driven world-time advancement with malformed-value fallback and minimum 1-minute advance, `noProse` execution mode, and deterministic summary wording with `from`/`using` source context); references shared payloads in `docs/api/common.md`.
- [api/factions.md](api/factions.md) — Faction listing/CRUD, player standings endpoints, and AI fill-missing endpoint for partial faction creation payloads (including robust relation/tier/asset format parsing, quote-insensitive relation name mapping, and detailed autofill-missing errors).
- [api/game.md](api/game.md) — Game lifecycle endpoints (including new-game form settings save/load/list APIs, start-time hour input handling, save-path server logging on successful saves/autosaves, calendar generation/regeneration behavior with Earth-like Gregorian prompt guidance plus season/holiday/time-of-day descriptions, and optional unspent attribute point input); references shared payloads in `docs/api/common.md` and notes settings-sourced skills for new games.
...
- [api/players.md](api/players.md) — Player and party endpoints (including level-up chat logging with base-context history exclusion, plus admin stats updates with runtime registration of newly submitted skill names and definition-based attribute validation in `/api/player/update-stats`); references shared payloads in `docs/api/common.md`.
...
- [classes/LLMClient.md](classes/LLMClient.md) — LLM chat client with concurrency, streaming, retries, retry-time config/override refresh, prompt logging, cancellation utilities, and request payload notes.
