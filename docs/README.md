...
| [config.md](config.md)                                 | Runtime/gameplay configuration, overrides, model settings, staged tiny-brain player actions, assistant prefill, system-prompt append text, tracker limits, retry waits, global prompt concurrency, cache-stable base-context inclusion/tool schemas and recent-story message checkpoints, staggered prompt timing, prompt behavior, and mod enablement. |
...
| [ui/chat_interface.md](ui/chat_interface.md)   | Main Play interface behavior, tabs, panels, chat flow/error alerts including committed-travel arrival errors, model-bound chat request filtering, prompt tracker dock and active-prompt favicon progress including chronological prompt-viewer messages, chat-log action controls including in-flight input edit ids, cookie-persisted chat bubble type filtering, and parent-linked turn-diff pruning, inventory lists including AI-backed item search, mobile-safe shared item filters with inventory-modal refresh behavior, mobile two-column modal slot grids, and mechanics-aware stack combining, tracker sidebar cards/progress fills/hover details/top-layer overlay controls, Story Tools, entity image upload/prompt editing/saved-prompt regeneration, concurrent distinct dragged-item operations, mobile touch-drag operations, and client data flow. |
| [ui/modals_overlays.md](ui/modals_overlays.md) | Chat-page modal, shared Ctrl/Cmd+Enter submission, overlay, tooltip, background summary-failure alerts, prompt tracker and default-follow persistent grouped prompt viewers, entity image upload/prompt editing fields, alphabetized ability displays and level-up draft, item stack combiner, container, crafting touch-drag, barter, blank stub descriptions, location-region fixer, and map confirmation behavior.                                      |
...
| [classes/LLMClient.md](classes/LLMClient.md)                               | LLM completions, backend selection, cache-stable base-context tool policy, custom payload forwarding, assistant prefill, grouped streaming progress/failure display, retries, cancellation, logical prompt queue reservations, logging, and prompt stats.   |
...
| [classes/TinyBrainPromptRunner.md](classes/TinyBrainPromptRunner.md)       | Staged player-action Nunjucks checkpoints, boundary-aware N/A and branch parsers, persistent progress viewer grouping, clean in-place retries, tool-result retention, and single-file transcript logging.       |
...
| [classes/base_context_history.md](classes/base_context_history.md)         | Base-context history inclusion/exclusion plus stable scene-coverage partitioning for regular and generic prompts.                                                                        |
