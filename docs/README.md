...
| [server_llm_notes.md](server_llm_notes.md)             | Detailed server, cache-stable non-generic base-context tool schemas, prompt helpers including compact `quantity N` item labels, LLM, chat tools, event, travel, save/load, and generation flow notes including generation-only random integer tool access. |
| [config.md](config.md)                                 | Runtime/gameplay configuration, overrides, model settings, maintenance-prompt intervals, staged tiny-brain player actions, assistant prefill, system-prompt append text, tracker limits, retry waits, global prompt concurrency, cache-stable base-context inclusion/tool schemas, shared-prefix message checkpoints, and player-action slop-context reuse, staggered prompt timing, prompt behavior, and mod enablement. |
...
| [slop_and_repetition.md](slop_and_repetition.md)       | Slop-word, ngram, regex, forbidden-trope, turnResult parsing, repetition-removal systems, and the cache-stable base-context slop-remover path.                |
...
| [api/chat.md](api/chat.md)                       | `/api/chat`, history, prompt modes, model-bound request filtering, prompt-render and background-summary failure handling, shared non-generic base-context tool schemas, chat tools and context-lookup guidance including tracker notes/event/relationship/quest creation, configurable parser-based silent housekeeping with optional tool-call debug entries including single merged housekeeping for split travel, event results, travel with cross-region exit travel-time backfill, while-you-were-away candidate synthesis, time, and streaming payloads. |
...
| [api/game.md](api/game.md)                       | New/load/save lifecycle, CLI startup save loading, new-game runtime reset across world maps/static indexes/async work including maintenance-prompt counters, mod mismatch handling, active calendar, config overrides, and game state endpoints. |
| [api/images.md](api/images.md)                   | Image generation, persisted entity prompts, saved-prompt regeneration, editable prompt generation, image jobs, uploads, descriptive browser-save filenames, serving, prompt batching, and weather/lighting variants.                                  |
| [api/locations.md](api/locations.md)             | Locations, exits, stubs, relocation, region-membership conflict repair, favorites, modification, generated things/NPCs, and deletion.                                  |
...
| [classes/ComfyUIClient.md](classes/ComfyUIClient.md)                       | ComfyUI image-generation queueing, upload, polling, result download client, workflow-template payload notes including config access, vanilla primitive prompt text nodes, Crystools prompt display nodes, and bundled workflow dependencies including Krea 2 LoRA chaining.      |
| [classes/Events.md](classes/Events.md)                                     | Event-check, persisted maintenance-prompt intervals including objective-signal acceleration, staggered background checks, and concurrent housekeeping XML parsing/application for world mutations, travel, time, quests, needs, trackers, relationships, and summaries. |
...
| [classes/LLMClient.md](classes/LLMClient.md)                               | LLM completions, backend selection, sectional cache-checkpoint boundaries, cache-stable base-context tool policy, custom payload forwarding, assistant prefill, safe SSE/transport completion including optional `Connection: close`, stable grouped progress with inter-stage waiting/failure display, retries, cancellation, logical prompt queue reservations, logging, and prompt stats.   |
| [classes/Location.md](classes/Location.md)                                 | Location model, persisted image prompts, stubs, visits, favorites, exits, things/NPCs, optional generated metadata, weather hints, region membership, and travel routes.           |
...
| [classes/Thing.md](classes/Thing.md)                                       | Item/scenery model, persisted image prompts, stacks, containers, mod fields, effects, values, checksums, and placement.                                                           |
| [classes/TinyBrainPromptRunner.md](classes/TinyBrainPromptRunner.md)       | Shared staged player-action/event/future-prompt Nunjucks checkpoints, boundary-aware N/A and branch parsers, runner-owned queue reservation, one cumulative asymptotic progress row with a whole-run average and blue inter-stage waiting, clean in-place retries, tool-result retention, and single-file transcript logging.       |
