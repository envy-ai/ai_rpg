...
| [config.md](config.md)                                 | Runtime/gameplay configuration, overrides, model settings, staged tiny-brain player actions, assistant prefill, system-prompt append text, tracker limits, retry waits, global prompt concurrency, staggered prompt timing, prompt behavior, and mod enablement. |
...
| [ui/chat_interface.md](ui/chat_interface.md)   | Main Play interface behavior, tabs, panels, chat flow/error alerts including committed-travel arrival errors, model-bound chat request filtering, prompt tracker dock and active-prompt favicon progress, chat-log action controls including in-flight input edit ids, cookie-persisted chat bubble type filtering, and parent-linked turn-diff pruning, inventory lists including AI-backed item search, mobile-safe shared item filters with inventory-modal refresh behavior, mobile two-column modal slot grids, and mechanics-aware stack combining, tracker sidebar cards/progress fills/hover details/top-layer overlay controls, Story Tools, entity image upload/prompt editing/saved-prompt regeneration, concurrent distinct dragged-item operations, mobile touch-drag operations, and client data flow. |
...
| [classes/LLMClient.md](classes/LLMClient.md)                               | LLM completions, backend selection, custom payload forwarding, assistant prefill, grouped streaming progress/failure display, retries, cancellation, logical prompt queue reservations, logging, and prompt stats.   |
...
| [classes/SceneSummaries.md](classes/SceneSummaries.md)                     | Scene summary storage, range indexing, non-blocking invalid-save recovery, Story Tools edits, and scene lookup/rerun tools.                               |
