...
- With the `scheduled_event_resolution` TinyBrain family enabled, the prompt separately decides applicability, may run read-only lookup tools, produces a parser-validated call plan, approves a hidden summary, and drafts/audits visible prose when the player is present. The plan does not echo the raw event, declare a change flag that code can derive, provide both an id and name for one target, or explain a purpose after its exact call is already known. It supplies one target reference plus an allowlisted field/JSON value for each direct update, and the exact tool name/arguments for every other call. The current event's lifecycle and history record are server-owned. An observational or history-only event with no supported game-state mutation therefore uses an empty plan instead of inventing a quest, memory update, or other call merely to record, replay, or reschedule it; `scheduleEvent` remains available for a distinct future consequence. Identical operations collapse safely while conflicting updates to the same field fail. Tool-specific semantic preconditions are enforced while parsing the plan when they can be checked without game-state lookup. The runner's post-parse hook immediately executes every accepted call in order; there is no subsequent LLM checkpoint that asks the model to translate or repeat its plan as native tool calls. Successful operations are cached for the resolution and reused when an execution failure sends the complete plan checkpoint back for correction, while structured tool failures are not cached. A failed invocation therefore retries the plan itself; if retries are exhausted, the request fails loudly and the scheduled record remains pending rather than being marked resolved or skipped. Any world-time adjustment that made the event due has already occurred and is not transactionally rolled back. A local result builder then assembles final XML from the approved summary and prose. Both prompt variants state that their base context describes the scheduled location and that player presence controls visibility only: a feasible event must not be skipped merely because the player is elsewhere or the affected object is not in player inventory.
...
- Scheduled-event resolution receives the built-in and registered mod chat-tool list, including ordinary world-mutation tools, but excludes generic-prompt-only Story Tools (`editChatLogEntry`, `rerunSceneSummary`, and `editSceneSummary`). Silent event-check housekeeping uses the same non-generic mutation-capable scope and additionally verifies that `createQuest`, `updateTracker`, and `removeTracker` are present before calling the model.
- `requestUserInput` is in the scheduled-event tool list. It succeeds only with an active client stream/id, realtime delivery, and enabled configuration; otherwise server-owned plan execution records a visible tool error for the next checkpoint.
