...
- With the `scheduled_event_resolution` TinyBrain family enabled, the prompt separately decides applicability, plans changes, exposes tools only for one execution checkpoint, approves a hidden summary, drafts/audits visible prose when the player is present, and returns strict final XML. One shared tool cache carries completed results across parser retries. The final parser requires the approved summary verbatim and forbids player prose for offscreen events.
...
- With the `scheduled_event_interruption_rewrite` family enabled, the rewrite is planned, drafted, audited, and parsed in one staged conversation. Its invariant parser permits changes only beneath player-facing prose tags and rejects any root, hidden-note, duration, destination, vehicle, or other non-prose drift.
