...
- The `mystery-box-update` prompt can create, update, or skip. Create/update responses must attach the box to a `MysteryThread`; the thread is created or updated through `MysteryThread` and active-thread creation/activation is bounded by `mystery_threads.max_active`. When active capacity is full or zero, unrelated mentions can return `skip`.
