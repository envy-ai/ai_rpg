...
- When `LLMClient` supplies progress callbacks, the bridge forwards app-server notifications back into the existing prompt-progress system. It now converts structured-output JSON deltas like `{"content":"hello"}` into plain assistant preview text (`hello`) before emitting the preview events, so the popup streams real reply text instead of raw bridge JSON.
