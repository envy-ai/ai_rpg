...
`Region.fromXMLSnippet` accepts `includeSecrets` (default `true`). Server region generation passes `config.regions.secrets_enabled === true`; when false, unsolicited `<secrets>` are ignored, and missing secrets are valid. Pending-region parsing applies the same switch. This option affects generated XML only; save hydration and `toJSON()` preserve existing secrets regardless of configuration.
...
- Chat tools: `updateObjectFields({ objectType: "region" })` can update allowlisted scalar/list/object fields such as descriptions, level metadata, concepts, secrets, visit time, parent/entrance/faction ids, weather, weather state, random events, and status effects. Compact `moreInfo` omits bulky region scaffolding like `locationBlueprints`, random events, concepts, and `weatherState` unless `includeFullState` is requested, while leaving `secrets` visible only when `regions.secrets_enabled` is true.
- Prompt context: base context renders current region name, description, location names, and connected region names. Region secrets are included only when `regions.secrets_enabled: true` (default false), including region NPC generation, short-description prompts, and region tool results. `moreInfo` omits secrets while disabled even for full-state requests. Current weather enters prompts through the world-time payload after `resolveCurrentWeather()`. `templates/region.njk` renders fuller region detail including parent, controlling faction, average level, locations, connected regions, enabled secrets, current weather, and status effects.
