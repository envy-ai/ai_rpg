...
`TinyBrainPromptRunner.js` executes the optional staged player-action prompt used when `ai.tinybrain: true`. It lets a small model answer one reasoning instruction at a time while preserving the conversation, tool calls, and tool results for the later draft and final-response steps.
