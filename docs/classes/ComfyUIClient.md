...
| `unloadModels()` | POSTs `{ unload_models: true, free_memory: true }` to `/free`. It throws when ComfyUI cannot confirm the request. |
| `releaseVram()` | POSTs `{}` to Cache Monitor's `/comfyui-cache-monitor/release_vram` and requires a response with `released: true`. Cache Monitor partially unloads active model weights from VRAM while retaining their registered RAM-backed caches. If that request fails or does not confirm release, the method calls `unloadModels()` as a full `/free` fallback and returns `fallbackUsed: true` plus the Cache Monitor error. If both requests fail, it throws an `AggregateError` containing both failures. |
...
When an effective AI model enables `unload_during_image_generation`, the server calls `unloadModels()` before every real LLM transport attempt. This pre-prompt call uses full cleanup and is strict: failure prevents the text request from starting. The server initializes a cleanup-only ComfyUI client for this purpose even when image rendering is disabled. After all queued renders and downloads finish, the image-render lifecycle instead calls `releaseVram()` immediately before it reloads the llama.cpp text model. Cache Monitor preserves the registered RAM-backed cache when available. If its endpoint fails, `releaseVram()` completes full `/free` cleanup, and the lifecycle broadcasts a browser warning recommending Cache Monitor installation. Cleanup failure remains optional in router mode so it does not prevent the reload attempt.

When `terminate_during_image_generation` owns a local llama.cpp process, the server also performs ComfyUI cleanup immediately before startup-script execution. Initial startup and ordinary model-script switches use `unloadModels()` for full cleanup. The restart after a completed or failed render batch uses `releaseVram()` to preserve Cache Monitor's RAM-backed cache, falling back to full cleanup and the same browser warning when necessary. Cleanup is strict in termination mode; failure of both Cache Monitor and `/free` prevents the script from running and keeps the text lifecycle gate closed.
...
- `getImage()`, `uploadInputImage()`, `unloadModels()`, and `releaseVram()` throw request/validation errors.
