...
| `releaseVram({ signal } = {})` | POSTs `{}` to Cache Monitor's `/comfyui-cache-monitor/release_vram` and requires a response with `released: true`. Cache Monitor partially unloads active model weights from VRAM while retaining their registered RAM-backed caches. If that request fails or does not confirm release, the method invokes its private full-cleanup helper, which POSTs `{ unload_models: true, free_memory: true }` to `/free`, and returns `fallbackUsed: true` plus the Cache Monitor error. No public method exposes `/free`. If both requests fail, it throws an `AggregateError` containing both failures. `signal` aborts either request immediately; cancellation throws `AbortError` and never triggers `/free` as an error fallback. |
...
When `imagegen.batch_prompts` is enabled, the server groups ordinary jobs by effective workflow and exact resolution, renders each group once with an `images` template array, and fans the shared Comfy prompt id back out to each game job. ComfyUI only exposes the saved output list after that workflow finishes, so the server aggregates each sampler node's repeated list-item passes into one monotonic overall batch percentage and publishes it to every member job. It strictly requires one returned image per prompt and maps results in list order. Each mapped result still receives its own generated image id, metadata record, entity attachment, and completion event. Location weather variants remain single img2img submissions.
...
Location weather/lighting variants are img2img jobs. They require `imagegen.engine: "comfyui"` and a usable base location image. They use the canonical `imagegen.workflow.edit` selection (Flux Klein by default), including its optional custom template. The server builds the deterministic edit prompt locally, uploads the base image with `uploadInputImage()`, renders the variant workflow, and stores the completed image as display-only variant metadata on the location without replacing `location.imageId`.
...
When an effective AI model enables `unload_during_image_generation`, the server calls `releaseVram({ signal })` before every real LLM transport attempt. This pre-prompt call preserves Cache Monitor's registered RAM-backed cache and is strict: if Cache Monitor and its `/free` fallback both fail, the text request does not start. Stop & Undo aborts that signal, ending pre-prompt cleanup without launching the text transport or treating cancellation as a reason to call `/free`. The server initializes a cleanup-only ComfyUI client for this purpose even when image rendering is disabled. After all queued renders and downloads finish, the image-render lifecycle also calls `releaseVram()` immediately before it reloads the llama.cpp text model. If Cache Monitor fails, the method completes full `/free` cleanup and the lifecycle broadcasts a browser warning recommending Cache Monitor installation. Cleanup failure remains optional in router mode so it does not prevent the reload attempt. Managed local llama startup treats an unreachable ComfyUI server specially: it warns and continues, because no unreachable instance can be retaining GPU models; reachable cleanup errors remain fatal.

When `terminate_during_image_generation` owns a local llama.cpp process, the server also performs ComfyUI cleanup immediately before startup-script execution. Initial startup, ordinary model-script switches, and post-render restarts all use `releaseVram()` to preserve Cache Monitor's RAM-backed cache, falling back to full cleanup and the same browser warning only when necessary. Cleanup is strict in termination mode; failure of both Cache Monitor and `/free` prevents the script from running and keeps the text lifecycle gate closed.
...
- `getImage()`, `uploadInputImage()`, and `releaseVram()` throw request/validation errors; an aborted `releaseVram({ signal })` throws `AbortError` without entering its cleanup fallback.
