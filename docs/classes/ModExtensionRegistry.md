...
- `registerEntityField({ entityType, fieldName, type, defaultValue, description, exposeToCreateTool, exposeToUpdateTool, exposeToGeneratorPrompt, exposeToXmlParser, exposeToEditModal, clearThingSlotWhenPresent, edit, xmlPrompt })`: declares a first-class mod-owned entity field. The initial supported entity type is `thing`. Registered Thing fields are persisted at top level by `Thing.toJSON()`, exposed through `thing.getExtensionField(...)` / `thing.setExtensionField(...)`, and installed as direct instance accessors such as `thing.implantSlot`. `exposeToEditModal` renders the field in the shared item editor using `edit` metadata, and `clearThingSlotWhenPresent` clears the built-in `Thing.slot` equipment field when the registered field has a meaningful value.
...
## Notes
- Duplicate chat tool names, XML tags, XML event keys, setting tabs, setting fields, entity fields, Thing image badges, and Thing context actions fail loudly.
