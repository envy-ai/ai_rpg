...
- Each chunk renders `prompts/scene-summarize.xml.njk`. With `ai.tinybrain_prompts.scene_summarize` enabled under the master TinyBrain switch, the wrapper runs the staged `prompts/_includes/scene-summarize.tinybrain.njk` program; otherwise it makes the existing one-shot `LLMClient.chatCompletion(...)` call with `metadataLabel: 'scene_summarize'`, `runInBackground: true`, and whole-response XML validation disabled.
- Before each render, enabled mods may add chunk-specific guidance through `scope.registerSceneSummarizeContributor(fn)`. Contributors receive `{ fullHistoryLines }`, omit themselves with an empty/null result, and render in both the one-shot detail guidance and each TinyBrain scene-detail checkpoint; non-string results fail the render explicitly.
- The staged program first proposes and then verifies strictly ascending scene-start boundaries. It requests one parser-validated summary for each completed boundary interval, then a local result builder assembles the ordinary `<scenes>` response and adds the final following-scene marker expected by the existing downstream parser. No extra formatting completion is required.
- One-shot prompt and response data are logged under the `scene_summarize` prefix. TinyBrain checkpoints, retries, and the locally assembled result share one incremental `scene_summarize_tinybrain` log.
