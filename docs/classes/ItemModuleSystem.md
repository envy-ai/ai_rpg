...
- `install({ actor, baseItem/baseItemName/baseItemId, moduleItem/moduleItemName/moduleItemId, slotType, slotTypes, registerSplitItem?, reason })` installs a module in the next open compatible slot and writes both sides of the relationship. The helper requires an actor-like owner with `getInventoryItems()`; item objects, ids, or names must resolve inside that owner. It rejects missing stable ids, self-installation, equipped module items, already-installed modules, wrong requested slot types, full slots, and stale installed-module references. If either selected Thing is stacked, the helper splits one single-count instance from that stack: the installed base and installed module are each `count: 1`, while each original stack remains loose with its count reduced by one. The optional registrar records newly split Things in the runtime registry; all bundled mod callers supply it.
...
- A stacked base item that already contains module links is invalid legacy state and is rejected rather than copying one module id onto multiple item instances. Ordinary Separate and Split Stack actions likewise require removing any installed module relationship first.
