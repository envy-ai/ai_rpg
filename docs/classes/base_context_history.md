...
- Manually deleting any covered scene-summary source entry is normal history editing, not data corruption. Rendering warns once and temporarily treats all remaining history as uncovered so deleted text cannot survive through a stale derived summary. The manual-delete API clears derived scene summaries when a covered source entry is removed, and scheduled scene summarization performs the same stale-source check before calculating its next range.
