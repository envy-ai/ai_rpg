...
- `hydrateGameState` includes legacy save migration passes that convert hour-based fields to minute-canonical data (`worldTime`, elapsed/visited timestamps, status-effect duration/appliedAt, weather duration fields, and offscreen scheduler snapshots) and that scale pre-`1.1` saved player/NPC need-bar values by `10`, then bump the in-memory save metadata version to `1.1` so the migration does not reapply after the next save.
