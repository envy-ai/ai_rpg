...
- The reactive `mysteryBoxMention` flow runs a private `mystery-thread-check` prompt before `mystery-box-update`; matching returned active thread names are marked `inactive`, and matching returned mystery-box names are marked resolved. Resolved boxes stay in `boxIds` for continuity/editing but are omitted from active-thread base context even when their parent thread remains active. Unknown thread/box names are console-warned and ignored.
- The generic-prompt-only `updateMysteryThreadFields({ mysteryThread, fields })` chat tool directly edits `name`, `status`, `keys`, `summary`, and/or `constraints` for one thread resolved by id/name/key/alias. It uses replacement semantics like Story Tools edits: submitted `keys` and `constraints` replace existing lists, removed aliases stop resolving, and contained `boxIds` are preserved.
