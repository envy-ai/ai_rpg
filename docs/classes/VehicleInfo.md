...
- A destinationless zero-minute vehicle update means an unplanned stop. It requires an underway vehicle, clears `pendingDestination`, `ETA`, and `departureTime`, and does not retarget the vehicle exit or fabricate an outside arrival.
- While a vehicle is underway, its tracked outside exit still identifies the departure location but remains hidden. Redirecting back to that location with positive travel time starts a fresh timed trip; it must not be mistaken for an immediate arrival merely because the hidden exit already points there.
- `Player.currentVehicle` wraps active location/region vehicle data with prompt/display fields such as `destination`, `destinationResolved`, `minutesToDestination`, `timeToDestination`, `isUnderway`, `hasArrived`, and `isArriving`. Both countdown fields are `null` when no ETA is set; an unset ETA is never coerced to minute zero.
- Location and region API payloads enrich embedded `vehicleInfo` with display-only fields: `isUnderway`, `hasArrived`, `isArriving`, `minutesToDestination`, `timeToDestination`, `tripCompleteFraction`, `destinationResolved`, and `displayDestination`. The countdown fields are `null` when the trip has no ETA.
