...
- `<turnResult>` requires a direct `<prose>` child for the visible story text. Nested `<hidden>...</hidden>` note blocks inside `<prose>` are preserved in place, and direct child `<hidden>...</hidden>` siblings are appended to the parsed final prose. The canonical direct-child `<timePassed>` metadata is parsed into minutes and excluded from visible prose. As a narrow compatibility case, one `<timePassed>` element placed directly inside `<prose>` is parsed identically and removed; duplicate or malformed timing blocks fail explicitly.
- `<moveTurnResult>` contains optional `<vehicleInfo>`, optional `<playerDestination>`, and at least one of `<originProse>`, `<betweenProse>`, or `<destinationProse>`. One misplaced `<timePassed>` directly inside a movement prose segment is validated and removed from prose, but is not applied because route/exit or travel metadata remains authoritative and applying both would double-count travel time.
