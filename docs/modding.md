...
- `mods/need-bar-lust`: defs-only mod that contributes the `sex` / `Sexual Satisfaction` need bar.
- `mods/nsfw-boost`: runtime/defs mod that registers the first-class `Player.sexualTraits` field, contributes a `<sexualTraits>` reference list from its mod-owned `defs/sexual_traits.yaml`, exposes the field to NPC generation and alteration XML, and adds ordinary and tiny-brain player-action guidance for sexually proactive NPC behavior.
