...
- Hybrid mods may contain both `mod.js` and `defs/`; their runtime hooks load normally and their defs overlays participate in the same deterministic merge order.
...
- `mods/need-bar-lust`: hybrid need-bar mod that adds the `sex`/`Sexual Satisfaction` need bar through defs overlays and registers step-1 player-action prompt guidance for lust-driven NPC initiative.
- `mods/modules`: item-to-item module slots for equippable gear. It registers `moduleSlots`, `moduleType`, `installedModuleIds`, and `moduleInstalledOnItemId`, adds a Modules World Profiles tab with configurable slot-type rows and Module/Crystal/Mod/Materia presets, exposes a structured `moduleSlots` `createThing` schema requiring `type`, renders installed modules on item cards/tooltips, contributes installed-module attribute/status/target effects, and adds item-generation guidance when module items lag modular base items.
