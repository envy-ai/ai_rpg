...
- Interaction API
  ...
  - `interaction.adjustWorldTimeByMinutes(minutes, { source? })` is available when the command needs to move the world clock through the shared minute-based time path; positive values use normal forward advancement and due-arrival processing, while negative values perform a raw rewind without undoing previously processed time-based side effects.
