...
- `checkResults`: array | undefined (structured records on visible `check-results` entries; each record includes `kind` (`skill`, `opposed-skill`, or `attack`), `status`, one-line `summary`, optional `skillCheck` or `attackSummary` expanded-detail payload, and `cacheHit`/`cacheKey` for cached tool results. Completed collapsed summaries avoid roll/DC/margin/damage/health numbers; those stay in the expanded payloads. Skill summaries prefix the icon mapped from the existing outcome `degree` (`💣` critical failure, `🧨` major failure, `❌` failure, `😞` barely failed, `😰` barely succeeded, `✔️` success, `⭐` major success, `🌟` critical success); attack summaries map the existing hit/miss result to `✔️`/`❌`.)
