...
- The deterministic edit prompt is rendered locally from `templates/location-weather-variant-image-prompt.njk`, or `templates/location-weather-variant-interior-image-prompt.njk` for locations with no visible outdoor weather; no LLM prompt-writing call is made for this endpoint.
