...
- Thing/item image generation is no longer limited to the player inventory or current location; any known thing can request a job when it renders without an existing usable image.
- Before jobs are queued, LLM-authored image-prompt generation retries up to `imagegen.prompt_generation_attempts` times, default `3`. Empty prompt text, leaked prompt/context XML, and prompt wrapper text are treated as prompt-generation failures. If all attempts fail, the request is skipped with `reason: "image-prompt-failed"` and no image-rendering job is queued.
- LLM-authored image-prompt generation requests are debounced and batched when `imagegen.prompt_batching.enabled` is true. The queue waits `imagegen.prompt_batching.delay_ms` after the most recent compatible request and sends up to `imagegen.prompt_batching.max_items` prompts per batch. The final image-rendering jobs are still queued independently.
