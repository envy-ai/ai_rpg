...
Image jobs use the statuses `queued`, `processing`, `completed`, `failed`, and `timeout`. `imagegen.maxConcurrentJobs` controls concurrent processing. When a request includes `clientId`, the server subscribes that client to `image_job_update` realtime events for the queued or joined job. Job snapshots and realtime updates include `isRendering`, `renderProgress`, and `renderNodeId`. For ComfyUI, `renderProgress` is the active sampler node's `value / max` percentage and may reset when a workflow advances to another sampler; it is not aggregate workflow completion.
