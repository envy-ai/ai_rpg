...
- The route refreshes expired trade refusals, performs daily barter-stock refresh when enough world time has passed, renders the base-context `barter-prices` prompt, logs every returned attempt through `LLMClient.logPrompt` with metadata label `barter_prices`, and stores a temporary quoted session.
- Completion and strict pricing parsing share the configured `ai.retryAttempts` budget. A syntactically or semantically invalid returned block is retained in the private retry transcript with concise parser feedback, then only the pricing prompt is rerun. The parser enforces the effective generated-stock minimum/maximum supplied to that pass; haggle passes use an exact `0..0` bound. Merchant currency, newly generated stock, parsed offers, and the session are applied only after an accepted response.
...
- After pricing passes strict parsing, adds exactly one `barter-haggle` chat entry for the player's offer and one for the merchant's response text. Parser retries do not duplicate either entry, and an exhausted invalid response creates neither. NPC speaker labels for the trade modal are stored in session history/client rendering rather than prepended to stored response text.
...
Request:
- Body: `{ locationId: string, accountTravelTime?: boolean, storyToolTeleport?: boolean, arrivalProseAlreadyProvided?: boolean, accompanyingCharacters?: string[], clientId?: string }`
...
- For gameplay player teleports, `arrivalProseAlreadyProvided` must be a boolean and defaults to `false`. A nonblank map/Favorites action sends `true`; a blank confirmation that stored only a movement comment sends `false`. When a qualifying WYWA pass has no earlier arrival prose, it requires nonempty `proseForPlayer`; already-narrated arrivals retain optional WYWA prose.
