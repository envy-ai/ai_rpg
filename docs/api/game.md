...
- If the loaded current setting lacks `hidingAttribute` or `perceptionAttribute`, `/api/load` runs the `setting_hide_perception` prompt at game-load time, choosing required attributes from the defined attribute list and optional skills from the setting's `defaultExistingSkills`. Invalid or missing required prompt results fail the load loudly. Successful backfills update `currentSetting` and rewrite the loaded save so later loads keep the selections.
