...
- Party-memory `npc_memories` prompts run as post-response background work. They skip dead NPC/corpse actors, can add memories, update goals/dispositions, and emit `chat_history_updated` if a disposition summary entry is created, but the chat/travel HTTP response no longer waits for those prompts to finish.
