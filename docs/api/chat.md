...
- Regular prose tools (mechanical check tools are omitted when `use_legacy_prompt_checks: true`):
  - `moreInfo({ name, type? })`: returns pretty-printed JSON containing direct `toJSON`-style objects for matching NPCs (including alias matches), things, locations, and regions whose names contain the query substring. Optional `type` may be `character`, `thing`, `location`, or `region`; omitting it searches all categories. The JSON shape is `{ query, type, totalMatches, npcs, things, locations, regions }`.
