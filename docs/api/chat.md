...
- Tool availability is prompt-mode gated:
  ...
  - Enabled mods can add chat tools through `ModExtensionRegistry.registerChatTool(...)`. The tool list is assembled live for each request, so mods loaded after API route registration are still available when their `allowedInRegularProse` or `allowedInGenericPrompt` flags match the prompt type.
