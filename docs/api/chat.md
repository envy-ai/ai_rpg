...
- When a travel-prose vehicle destination requires creating a new destination location/stub, that creation is anchored to the vehicle's source location (not the player's current location). If that origin is a location vehicle, the helper-created plain origin exit is suppressed so the vehicle keeps only its dedicated vehicle exit to the destination, and that suppression intent now persists through later location-stub expansion as well.
