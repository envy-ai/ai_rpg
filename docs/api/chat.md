...
Split `<travelProse>` responses can include `<playerDestination><travelTime>...</travelTime></playerDestination>`. The duration is parsed with the shared duration parser and stored on a newly created origin exit for the player destination; if the relevant exit already exists, the prompt-provided value is ignored.
...
- Optional fields (present when relevant):
  ...
  - `checkResultsRecorded`: boolean (true when tool-derived skill/attack checks were recorded in a live prompt-excluded `check-results` chat entry, so clients should not duplicate them as legacy insight bubbles)

Variants:
- Empty player action: an empty final user message is accepted as a normal player-action continuation. The server skips plausibility and renders the player-action prompt with empty `actionText` so `prompts/_includes/player-action.njk` uses its "continue the previous scene" branch. By default, legacy attack precheck/check prompts and the automatic player-action plausibility prompt call are disabled in favor of tool-based checks; setting `use_legacy_prompt_checks: true` re-enables the legacy prompt flow for non-empty player/NPC actions.
...
- Inline die-roll override: player action text supports one or more `"<integer>"` tokens (pattern `/<-?\\d+>/`). These tokens are stripped from action text before prompt processing and before the user entry is persisted in chat history. Tool-based skill/attack checks consume the override during tool resolution; when `use_legacy_prompt_checks: true`, the legacy player-action plausibility/attack path consumes it before prose generation.
...
- Tool availability is prompt-mode gated:
  - Regular prompts (not prefixed by `@`, `@@`, `@@@`, or `\`) get information-gathering tools plus `scheduleEvent`, the health-mutating `resolveAttack` / `resolveAreaAttack` exceptions, and the skill-check resolution tools. This includes both `player_action` and `npc_action` prose prompts. When `use_legacy_prompt_checks: true`, regular prose prompts keep the information/lookup tools but do not receive `resolveAttack`, `resolveAreaAttack`, `resolveSkillCheck`, or `resolveOpposedSkillCheck`; attacks and checks are resolved by the legacy separate prompts instead.
  - Generic prompt actions (`@...`, `@@...`, `@@@...`) get the full tool set, including world-mutation tools. Their base prompt context also includes every stored chat log entry type with renderable text, including entries that ordinary prompts skip because of `metadata.excludeFromBaseContextHistory`.
  - No-context prompt actions (`\...`) do not get any chat tools.
  - Enabled mods can add chat tools through `ModExtensionRegistry.registerChatTool(...)`. The tool list is assembled live for each request, so mods loaded after API route registration are still available when their `allowedInRegularProse` or `allowedInGenericPrompt` flags match the prompt type. The bundled modules mod registers `installModule` and `removeModule` for inventory-backed item module installation/removal; base-context inventory lists hide installed modules as standalone items while the linked base item remains visible.
- Checked-container opening uses the regular-prose information tools through `prompts/_includes/player-action-open-container.njk` and always includes `resolveSkillCheck` / `resolveOpposedSkillCheck` for that specialized prompt, even when legacy prompt checks are enabled elsewhere. The route requires the model to call one of those skill-check tools, parses `<success>` and `<permanentlyOpened>` from `<containerOpenResult>`, stores the resulting prose visibly, and then runs the ordinary slop-removal and event-check path before the client decides whether to show the container inventory. Successful permanently-opened results persistently clear the container's `requiresCheckToOpen` flag.
- Regular prose tools (mechanical check tools are omitted when `use_legacy_prompt_checks: true`):
  - `moreInfo({ name, type?, includeFullState? })`: returns pretty-printed JSON for matching NPCs (including alias matches), things, locations, and regions whose names contain the query substring. Optional `type` may be `character`, `thing`, `location`, or `region`; omitting it searches all categories. By default `includeFullState` is `false`, so bulky/runtime fields such as character need-bar definitions, party memory history, region location blueprints/random events/weather state, and location image variants/stub/generation/event scaffolding are omitted from both model-visible content and tool metadata. Region `secrets` remain visible in compact output. Set `includeFullState: true` only for debugging raw `toJSON` payloads. The JSON shape is `{ query, type, totalMatches, npcs, things, locations, regions }`.
  ...
  - `hideEntity({ name, description? })`: sets `hiddenFromPlayer` on a resolved living NPC/entity and requests a location refresh. Dead NPCs/corpses remain visible. The prose model is responsible for running any needed opposed check first; this tool does not accept or perform a check.
  - `scheduleEvent({ event, region, location, in? | at? })`: persists a planned future event for a resolved region/location. Provide exactly one timing mode: `in` as a positive duration such as `"2 hours"` or numeric minutes, or `at` as canonical `{ dayIndex, timeMinutes }` from base context. Due events render `scheduled-event-resolution` with the complete built-in and mod chat-tool inventory, including world-mutation tools, then store a hidden `scheduled-event` chat-log summary; if the player is at the event location, a visible `scheduled-event-prose` entry is also stored and pushed to the client. For normal `<finalProse>` player actions with parsed `<timePassed>`, same-location due events inside the action interval are resolved before slop removal and folded into a rewritten player-action XML response instead of creating a separate visible scheduled-event prose row; off-location due events still resolve through the normal outside-prose sweep. `requestUserInput` is visible to scheduled-event resolution too, but still requires enabled config and an active client context to execute successfully.
...
- World-mutation tools (available for `@`/`@@`/`@@@` generic prompt actions and scheduled-event resolution):
  ...
  - `updateCharacterFields({ character, fields })`: resolves an NPC by ID/name and directly updates allowed persisted NPC fields without running the `alter_npc` event flow. Player characters are rejected. The allowlist is limited to identity/profile scalar fields, aliases, `attributes`, `skills`, `statusEffects`, `needBarApplicability`, and `willingToTrade`; equipment, inventory, barter inventory, party membership, quests, location, dispositions, abilities, and other object-graph fields are rejected before any update is applied.
  - `updateObjectFields({ objectType, object, fields })`: generic direct field patcher for `character`, `thing`, `location`, `exit`, `region`, `faction`, `quest`, `objective`, and `statusEffect`. It accepts IDs, exact names, and aliases where applicable; ambiguous names return `<toolError code="ambiguous_object">` with candidate `toJSON` payloads and instructions to retry with the exact ID. `character` still means NPC-only, while player-owned quests can be updated by ID/name and objectives by ID. Thing fields registered by enabled mods with `exposeToUpdateTool` are accepted at runtime, with the same active-setting dynamic descriptions used by `createThing`. Ownership, inventory, equipment, party, and other whole-object graph fields are not in the allowlists.
...
- Automatic summary-threshold checks, including scene-summary style saves, are scheduled fire-and-forget after post-turn random events/NPC turn handling. Summary prompt failures still warn to the server console, but the chat response is no longer held until the summary prompt finishes.
- Visible prose entries are pushed to the active client as soon as they are stored after slop removal when a target stream/client is available. This immediate refresh applies to player-action streaming plus stored NPC-turn prose, visible while-you-were-away prose, random-event prose, game intro prose, craft/process/salvage/harvest prose, location-modification prose, quest reward prose, and additional-effect prose; later event summaries and location updates can still arrive after that early prose refresh.
- Party-memory `npc_memories` prompts run as post-response background work. They skip dead NPC/corpse actors, can add memories, update goals/dispositions, and emit `chat_history_updated` if a disposition summary entry is created, but the chat/travel HTTP response no longer waits for those prompts to finish. On chat travel, the location-change memory pass only handles non-party NPCs left behind at the prior location; party members and departed party members are handled by the current-turn party-memory scheduler so one travel turn does not schedule both paths for the same actor.
...
- Plot analysis prompts run from `prompts/_includes/plot-analysis.njk` through `base-context.xml.njk` on normal/creative player-action prompts when `plot_analysis.enabled` is not `false` (the default is `true`). They are scheduled before the player-action LLM await and run fire-and-forget with `runInBackground: true`, so they do not block the turn. Only the newest scheduled completion is stored; stale completions from older turns are ignored. The latest result is stored in `Globals.plotAnalysis`, persisted as `metadata.plotAnalysis`, injected into normal base-context prompts as `<plotAnalysis>` only when it contains at least one plot thread or current plot complication, and intentionally omitted when rendering the `plot-analysis` prompt itself. `prompts/_includes/plot-analysis-blurb.njk` can render that same state as readable thread/complication lists, marking the current thread and warning when `plot_analysis.max_plot_threads` or `plot_analysis.max_plot_complications` is exceeded.
- `mysteryBoxMention` XML event outcomes first run a blocking private `mystery-thread-check` prompt when active mystery threads exist. It logs under `mystery_thread_check`, parses `<resolvedMysteries>`, marks returned matching active threads inactive so new unresolved mysteries can use the capped active slots, and marks returned matching mystery boxes resolved; unknown names warn to the console and are ignored. Resolved boxes remain persisted but are omitted from active mystery-thread base context even if the thread stays active. The flow then runs the private `mystery-box-update` prompt, which can call mystery-box and mystery-thread lookup tools, creates/updates a persisted `MysteryBox` note inside a `MysteryThread`, uses returned `<name>` as the canonical box key, updates parent thread summary/constraints, logs under `mystery_box_update`, and can return `<action>skip</action>` when active thread capacity is full and the mention is unrelated. It does not add visible chat entries or offscreen advancement by itself.
- When a player turn changes the player's location, the server also performs a blocking base-context `while-you-were-away` pass before the arrival scene is refreshed, but only if the destination was affirmatively snapshotted as visited before this move. First-time/unvisited arrivals, missing pre-arrival visit snapshots, and revisits with a known pre-arrival `lastVisitedTime` below `while_you_were_away_threshold_minutes` (default `30`) skip the prompt. `whileYouWereAwayNpcs` includes current-location NPCs with valid reunion last-seen data who were not with the player on the previous round, so already-present NPCs are not mistaken for arrivals; the location threshold does not filter individual NPC candidates once the prompt runs. The response can apply NPC need-bar `<value>` entries as absolute `0..100` percentages converted onto each target bar's raw min/max range after stripping nonnumeric text; parsed values outside that range are clamped to `0..100`, and blank/`N/A` values are skipped for that bar. It can optionally move those NPCs via structured `<travelDestination>` data, accepts additional arrival-only current-location updates marked with literal `HERE`, and resolves those arrivals against existing NPC names and aliases only. Unmatched `HERE` arrivals warn and skip without creating NPCs; duplicate matches prefer a single former party member, otherwise the first matching NPC in map order is used. If a `HERE` arrival resolves to a current party member at the player's arrival location, the NPC remains in the party and is not added to the location NPC list. After direct need/travel updates, the route runs a scoped event-check pass on the generated while-you-were-away update text and return prose; this suppresses the dedicated need-bar prompt, prevents player movement/time advancement, and hard-ignores `needbar_change` plus `npc_arrival_departure` while allowing other event categories to apply. The route always stores a hidden `while-you-were-away` chat entry linked to the main turn entry, and also appends a normal visible `while-you-were-away-player` assistant entry when the prompt returns non-empty `<proseForPlayer>`. When `slop_buster` is enabled, that visible prose uses the shared slop-removal pipeline before storage and may add the usual linked `slop-remover` attachment entry. Direct moves always persist a travel summary row and add elapsed time when exit time applies; they parent-link to visible arrival prose, prior travel prose, or the travel user/comment entry so direct travel renders as a `What changed` drawer. Map fast-travel summaries still parent-link to visible arrival prose when it exists.
...
Submits, confirms, or cancels a pending `requestUserInput` chat-tool request or an internal confirmation-mode request.
...
- Body:
  ...
  - `answer` (required string unless `cancelled: true`): non-empty answer text.
...
- Query options:
  - `includeAllEntries=true|false` (default `false`): when `true`, returns the full stored `chatHistory` in oldest-first order with no pruning and no hidden/orphan filtering. Intended for Story Tools/admin-style views. Player-action `<hidden>` note blocks are preserved in this full-history response only when `show_hidden_notes: true`.
