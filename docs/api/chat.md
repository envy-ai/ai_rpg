...
- World-mutation tools (available only for `@`/`@@`/`@@@` generic prompt actions):
  ...
  - `updateObjectFields({ objectType, object, fields })`: generic direct field patcher for `character`, `thing`, `location`, `exit`, `region`, `faction`, `quest`, `objective`, and `statusEffect`. It accepts IDs, exact names, and aliases where applicable; ambiguous names return `<toolError code="ambiguous_object">` with candidate `toJSON` payloads and instructions to retry with the exact ID. `character` still means NPC-only, while player-owned quests can be updated by ID/name and objectives by ID. Ownership, inventory, equipment, party, and other whole-object graph fields are not in the allowlists.
