...
- Body: SettingInfo fields (at minimum `name`)
  ...
  - Includes optional `modSettings`, a namespaced object used by enabled mod setting fields. Enabled mods may render those fields in their own World Profiles tabs, but the persisted payload remains this same object shape. Non-persisted mod action controls, such as preset-apply selects, update editable fields in the UI but are not included in `modSettings`. The bundled modules mod stores its configurable slot-type list under `modSettings.modules.slotTypes` as an array of `{ id, label, description }` entries.
...
- Body: SettingInfo fields
  ...
  - Supports `modSettings` updates. Invalid non-object `modSettings` payloads are rejected by `SettingInfo`; tab placement, select options, row editors such as the modules slot-type editor, and preset action controls are UI metadata from enabled mods and do not change the API shape.
