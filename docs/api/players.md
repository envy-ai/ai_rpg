...
- Direct moves persist a travel `event-summary` even when no time advances. When arrival prose is generated by the while-you-were-away prompt, the summary is parent-linked to that visible arrival prose; otherwise it attaches to the prior travel prose response or the travel user/comment entry so it still renders as a `What changed` drawer instead of a standalone events box.
