...
- The autofill prompt now includes an explicit required output schema and instructs nested XML-only output for `assets`, `relations`, and `reputationTiers` entries.
- Relation entries generated by name are resolved to existing faction ids server-side.
- AI relation parsing accepts both nested relation nodes (`<factionName>`, `<status>`, `<notes>`) and attribute-based relations (e.g. `<relation faction="..." status="...">notes</relation>`), with quote-insensitive target-name matching.
- AI reputation tier parsing accepts both nested tier fields (`<threshold>`, `<label>`) and attribute-based tiers (e.g. `<tier name="..." threshold="...">`).
- AI asset parsing accepts both structured assets (`<asset><name>...</name>...</asset>`) and compact assets (`<asset>Asset text</asset>`).
- When autofill is still incomplete, the error now includes specific missing fields and/or missing relation targets.
