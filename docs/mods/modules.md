...
- Thing context actions `modules:install-module` and `modules:remove-module` run through `POST /api/mod-thing-context-actions/:actionId`. Action payloads use `baseItemId`, `moduleItemId`, `slotType`, `baseItemSource`, and `moduleItemSource` to resolve player inventory, NPC inventory, and loose current-location items.
