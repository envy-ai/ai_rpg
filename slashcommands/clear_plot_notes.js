...
class ClearPlotNotesCommand extends SlashCommandBase {
  static get name() {
    return 'clear_plot_notes';
  }

  static get aliases() {
    return ['clear_plot_summaries', 'clear_plot_expander'];
  }

  static get description() {
    return 'Remove hidden plot-summary and plot-expander entries from chat history.';
  }
  ...
  static async execute(interaction) {
    ...
    let removedCount = 0;
    const removableTypes = new Set(['plot-summary', 'plot-expander']);
    ...
    if (removedCount === 0) {
      await interaction.reply({
        content: 'No plot summary or plot expander entries were found in chat history.',
        ephemeral: false
      });
      return;
    }
    ...
    if (typeof performGameSave !== 'function') {
      throw new Error('performGameSave is unavailable; cannot persist plot-note removal.');
    }
    ...
    await interaction.reply({
      content: `Removed ${removedCount} plot-note ${entryLabel} from chat history.`,
      ephemeral: false
    });
  }
}

module.exports = ClearPlotNotesCommand;
