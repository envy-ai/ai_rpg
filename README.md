...
6. Edit `config.yaml`:
   ...
   - Toggle `imagegen.enabled` or update the ComfyUI settings under `imagegen.server`. You can also set up external image generation that supports the OpenAI image generation API. Set `imagegen.maxConcurrentJobs` to allow parallel image processing (default: 1), or enable ComfyUI `imagegen.batch_prompts` with a list-capable workflow to group queued renders by exact resolution.
